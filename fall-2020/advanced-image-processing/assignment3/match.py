...
def get_transform(im1, im2, kps1, kps2, feats1, feats2, ratio=0.5, ret_idxs=False):
...
if __name__ == '__main__':
    ...
    j = i + 1
    im_dir = 'images/'
    im_fmt = 'painting%d.jpg'
    feat_dir = 'results'

    for ix in [i, j]:
        ims.append(imread(im_dir + im_fmt % ix))
        kps, feats = extract_or_load_features(ims[-1], feat_dir + '/kp_pyr%d.pkl' % ix,
                                              feat_dir + '/feat_pyr%d.pkl' % ix)
        kp_pyrs.append(kps)
